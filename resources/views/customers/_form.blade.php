<div class="grid gap-5 sm:grid-cols-2">
    ...
    @if($canEditInternalDetails ?? true)
        ...
        <textarea aria-label="Notes" id="notes" name="notes" rows="5" class="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3">{{ \App\Support\OldInput::scalar('notes', $customer->notes ?? '') }}</textarea>
        @error('notes')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
        </div>
    @endif
    @if(!isset($customer))<label class="flex items-start gap-3 rounded-xl border border-slate-200 p-4 sm:col-span-2"><input type="hidden" name="whatsapp_opt_in" value="0"><input aria-label="Whatsapp opt in" type="checkbox" name="whatsapp_opt_in" value="1" @checked(\App\Support\OldInput::scalar('whatsapp_opt_in', false)) class="mt-1"><span><span class="block font-semibold">Customer explicitly consents to WhatsApp</span><span class="text-sm text-slate-500">Leave unchecked unless consent has been affirmatively provided.</span></span></label>@endif
</div>
