<x-auth-layout title="Welcome back 👋" subtitle="Sign in to Akin Auto Parts">
    ...
    <form method="POST" action="{{ route('login.store') }}" class="inventra-login-form" x-data="{ submitting: false, reveal: false }" x-on:submit="submitting = true">
        @csrf
        <label class="inventra-login-field"><span>Email or phone</span><span class="inventra-login-control"><img src="{{ asset('images/figma/auth-mail.svg') }}" alt=""><input name="identifier" value="{{ \App\Support\OldInput::scalar('identifier') }}" placeholder="you@business.com" autocomplete="username" required autofocus></span>@error('identifier')<small>{{ $message }}</small>@enderror</label>
