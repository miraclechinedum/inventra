...
@php
    ...
    if (in_array($user->role, [\App\Enums\UserRole::Admin, \App\Enums\UserRole::Manager], true)) $nav[] = ['route' => 'reports.index', 'match' => 'reports.*', 'label' => 'Reports', 'icon' => 'dashboard'];
    // Only the roles that receive operational alerts today are offered the list; the route itself
    // stays open to any signed-in operator and is always scoped to their own deliveries.
    if (in_array($user->role, [\App\Enums\UserRole::Admin, \App\Enums\UserRole::Manager], true)) $nav[] = ['route' => 'notifications.index', 'match' => 'notifications.*', 'label' => 'Notifications', 'icon' => 'bell'];
    // One memoised read for the whole layout, however often the badge is rendered.
